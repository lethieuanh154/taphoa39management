...
export interface AiInvoiceData {
  ...
  sourceTab?: string;               // 'upload' | 'pdf' | 'email' | 'clone_image'
  gmailMessageId?: string;
  gmailFrom?: string;
  gmailDate?: string;
  portalUrl?: string;               // URL tra cuu HD tren portal NCC
  portalPdfUrl?: string;
  invoiceProvider?: string;         // 'VIETTEL' | 'VNPT' | 'MISA' | ...
  portalCredentials?: Record<string, string>;
  processingMethod?: string;        // 'flash' | 'pro' | 'xml_parse'
  attachmentType?: string;          // 'xml' | 'zip_xml' | 'pdf' | 'clone_image'
}
